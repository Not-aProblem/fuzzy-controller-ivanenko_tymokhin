...
def preprocessing(input_lvs, output_lv):
    ...
    for key, value in output_lv["terms"].items():
        mf, *params = value
        output_lv["terms"][key] = getattr(membership_functions, mf)(
            output_lv["U"], *params
        )
...
def process(input_lvs, output_lv, rule_base, crisp_values, metrick):
    ...
    crisp_result = defuzzifier.defuzzification(
        output_lv["U"], result_fuzzy_set, metrick
    )
    word_result = []
